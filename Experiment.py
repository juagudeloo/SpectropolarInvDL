...
def main():
    ...
    lr = 1e-3
    n_spec_points = 114
    test_stokes_weights = [[1,10,10,10],
                      [1,20,20,10],
                      [1,40,40,10],
                      [1,80,80,10],
                      [1,160,160,10]]
    ...
    for stokes_weights in test_stokes_weights:
        ...
        train(model=model,
            ...
            epochs=epochs,
            device=device,
            writer=create_writer(experiment_name=f"{stokes_weights[0]}_{stokes_weights[1]}_{stokes_weights[2]}_{stokes_weights[3]}_stokes_weights",
                                model_name=m_type,
                                extra=f""))
        
        #Save the model to file so we can get back the best model
        save_filepath = f"{m_type}_{stokes_weights[0]}_{stokes_weights[1]}_{stokes_weights[2]}_{stokes_weights[3]}_stokes_weights_spectral_points.pth"
